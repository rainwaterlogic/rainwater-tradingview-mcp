...
export function registerRainwaterTools(server) {
  server.tool('rainwater_chart_digest', 'Rainwater compact chart context: one fast call for symbol, quote, OHLC summary, study values, Pine levels/zones/labels, and token/speed estimates.', {
    mode: z.enum(['lite', 'standard', 'full']).optional().describe('Token/detail preset. lite is fastest/smallest, standard is the default, full includes more bars/items and tables.'),
    ...
    budget_tokens: z.coerce.number().optional().describe('Soft output budget. The digest progressively trims tables, drawings, and study detail if estimated output exceeds this budget.'),
